...
//
// NOTE: Internal code uses "show" terminology (show_number, shows table)
// but this represents "rounds" in the user-facing UI.
// RIVALRY_LENGTH is now 5 rounds.
...
// Rivalry length - should match config.js
const RIVALRY_LENGTH = 5;
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: previousShows } = await supabase
      .from('shows')
      .select('winner_id, show_number')
    ...
    // Determine conditions for emcee line selection
    let conditions = ['any'];

    if (triggerType === 'milestone') {
      conditions.push(`show_${showNumber}`);
    } else if (triggerType === 'show_transition' && showNumber > 1) {
      // Special end-game rounds for 5-round rivalry (rounds 4 and 5)
      // These map to the emcee_lines conditions in the database
      if (showNumber === RIVALRY_LENGTH - 1) {
        // Penultimate round (round 4) - use show_10 lines for drama
        conditions.push('show_10');
      } else if (showNumber === RIVALRY_LENGTH) {
        // Final round (round 5) - use show_11 lines for finale
        conditions.push('show_11');
      }
      
      // Game state conditions (apply to all rounds)
      ...
      // Blowout only possible with larger rivalries, skip for 5-round
    ...
    const { data: emceeLines, error: linesError } = await supabase
      .from('emcee_lines')
      .select('*')
    ...
    if (linesError || !emceeLines || emceeLines.length === 0) {
      ...
      const { data: fallbackLines } = await supabase
        .from('emcee_lines')
        .select('*')
        .eq('trigger_type', 'show_transition')
        .eq('condition', 'any');
      ...
      // Ultimate fallback
      return new Response(JSON.stringify({
        emcee_text: `Round ${showNumber}. Let's see what you've got.`
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
...
// NOTE: {show_num} placeholder shows the round number to users
function fillPlaceholders(
  ...
  return text
    ...
    .replace(/{show_num}/g, showNumber.toString())
    .replace(/{p1_score}/g, p1Score.toString())
    .replace(/{p2_score}/g, p2Score.toString())
    .replace(/{score}/g, p1Score.toString()) // For tied games
