...
interface SelectEmceeRequest {
  rivalryId: string
  showNumber: number
  triggerType?: string // Optional override for rivalry_intro
}
...
serve(async (req) => {
  ...
  try {
    const { rivalryId, showNumber, triggerType: overrideTriggerType } = await req.json() as SelectEmceeRequest
    ...
    } else if ([5, 10, 20].includes(showNumber)) {
    ...
    if (linesError || !emceeLines || emceeLines.length === 0) {
      ...
      if (fallbackLines && fallbackLines.length > 0) {
        ...
        return new Response(
          JSON.stringify({ emcee_text: filledText }),
      ...
      // Ultimate fallback
      return new Response(
        JSON.stringify({ emcee_text: `Show ${showNumber}. Let's see what you've got.` }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    // Fill placeholders
    const filledText = fillPlaceholders(
      selectedLine.text,
      player1,
      player2,
      showNumber,
